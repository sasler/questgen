...
topology, progression locks, and win path for each seeded game, while AI handles narration and
player help. The deterministic engine (`game-engine.ts`) validates all state changes against the
world graph. This prevents hallucinated exits, bypassed locks, impossible state transitions, and
unsolvable maps.
...
| `/showentitytables` | Print puzzle, lock, and win-condition tables, including what is still missing |
