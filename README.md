...
QuestGen uses the [GitHub Copilot SDK](https://github.com/github/copilot-sdk) for narration,
hints, and turn-by-turn play, while deterministic code generates the room graph and validates all
state changes. Each game gets its own stored seed, so the world stays reproducible inside that save
without sacrificing the absurd tone.
...
- 🧭 **Deterministic seeded world generation** — each game gets a reproducible, solvable room graph
- 🧱 **Explicit room interactables** — deterministic room objects keep stable IDs, aliases, and persistent state
- 🤖 **AI-driven narration and hints** — Copilot handles tone, prose, and player-facing help
- 🔒 **Deterministic game engine** — movement, direct interactions, and puzzle state resolve in code. No cheating.
...
**Key design principle:** The AI is NOT the game engine. Deterministic code now generates the
topology, progression locks, win path, and explicit interactable room objects for each seeded game,
while AI handles narration and player help. The deterministic engine (`game-engine.ts`) validates
movement, direct interactions, and state changes against the world graph plus authoritative
interactable state. This prevents hallucinated exits, fake targets, bypassed locks, impossible
state transitions, and unsolvable maps.
...
- Standard text adventure input works as usual: `look`, `go north`, `take key`, `use key on door`
- `/hint` asks the AI for a spoiler-bounded, actionable suggestion in the game's normal tone

### Hidden tester slash commands

These commands are intentionally **not shown in the site UI**, but they are useful when testing or
debugging a game:

| Command | Purpose |
| ------- | ------- |
| `/showfullmap` | Reveal the full generated room graph, ignoring fog of war |
| `/showentitytables` | Print richer interactable, puzzle, lock, and win-condition tables, including aliases, current state, and what is still missing |
| `/admin <question>` | Ask the admin/debug assistant about the authoritative game state, recent behavior, and AI/engine mismatches for the current save |

## World generation notes

- World topology, room graph shape, direction limits, and progression slots are generated by deterministic code from a per-game seed
- Story framing, room names, descriptions, items, NPCs, interactables, puzzle flavor, and win-condition flavor are AI-authored against that structural scaffold and then persisted with the save
- Generation includes a bounded AI review/repair loop so invalid or unsolved authored drafts are corrected before a game starts
- Interactable room objects are explicit world entities with persistent state plus stable IDs and aliases
- Each `Connection` record is a **single bidirectional corridor** and must not be duplicated with a
  mirrored reverse entry
- Movement resolves against the authoritative room graph, and direct interaction targets resolve against current-room interactables, not free-form model guesses
- Puzzle and lock data include explicit solution descriptions so hints and debug output can explain
  the next blocker clearly
